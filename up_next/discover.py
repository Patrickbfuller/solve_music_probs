...
def prep_main_artist_df(concerts_df, main_artist):
    ...
    main_df = concerts_df.loc[
        concerts_df['artist'] == main_artist, ['location', 'lat', 'lng']
        ]
...
class NeighborCitiesModel():
    """
    A nearest neighbor model for finding 1 closest city using haversine
    computation.
    """
    ...
    def find_closest_city(self, latlong):
        """
        Given lat long and find the closest city in a stored dataframe of
        cities.
        """
...
def get_neighbors_of_new_cities(concerts_df, main_artist, new_cities: list):
    """
    Given a list of cities where an artist has not been, return info on
    nearest cities where an artist HAS been. Input a list of strings for city
    names, return a list of dictionaries whose keys are: 'unplayed', 'played',
    'distance'. The values for 'unplayed' and 'played' are dictionaries with
    keys: 'city' and 'latlong'.
    """
    ...
    for new_city in new_cities:
        new_latlong = concerts_df.loc[
            concerts_df['location'] == new_city, ['lat', 'lng']
            ]
        new_latlong.drop_duplicates(inplace=True)  # can remove if un-duped

        played, played_ll, distance = neighbor_model.find_closest_city(
            new_latlong
            )

        unplayed_and_played.append(
            {'unplayed': {
                'city': new_city, 'latlong': list(new_latlong.values[0])
                },
             'played': {
                 'city': played, 'latlong': list(played_ll)
                 },
             'distance': distance}
            )
...
def pilfer_similar_artist(concerts_df, main_artist, similar_artist,
                          max_new_cities=None, cutoff_dist=None):
    """
    Given an main artist, and a similar artist, find info about cities where a
    similar artist has played but the main artist has not. Remove any cities
    that are close together(within cutoff). Return a list of new cities and
    their corresponding data, with closer cities appearing first.
    """
    if max_new_cities is None:
        max_new_cities = 5
    ...
    cities_and_nns = get_neighbors_of_new_cities(
        concerts_df, main_artist, new_cities
        )

    # Remove any new cities that are with in the cutoff distance
    relevant_cities_and_nns = [
        x for x in cities_and_nns if x['distance'] > cutoff_dist
        ]

    # Sort new cities by nearest to played
    cities_by_dist = sorted(
        relevant_cities_and_nns, key=lambda x: x['distance']
        )
...
class SimilarArtistModel():
    """
    Nearest Neighbors model for finding similar artists by playlist
    co-presence.
    """
    ...
    def __init__(self, playlists_df, concerts_df=None):
        ...
        encoded playlist flags and storing the dataframe.Option to pass in
        concerts df for find_similar_artist_venues() method at instantiaton.
    ...
    def find_artists_sim_to(self, main_artist, n_neighbors=None):
        ...
        """
        art1_idxs = self.artists.index[self.artists['artist'] == main_artist]
        art1_idx = art1_idxs[0]
        # sliced to extract from nested index object
        if n_neighbors is None:
            n_neighbors = 20
        self.distances, self.indices, = self.model.kneighbors(
            self.playlists, n_neighbors+1
            )
    ...
    def find_similar_artist_venues(self, main_artist, concerts_df=None,
                                   num_artists=None, max_new_cities=None,
                                   cutoff_dist=None):
        """
        ...
        if main_artist not in concerts_df['artist'].values:
            return f"""{main_artist.title()}:
               No Recent Shows OR Not In 20 Of Top Playlists"""
        try:
            results = []
            similar_artists = self.find_artists_sim_to(
                main_artist, n_neighbors=num_artists
                )
            for artist, cosine_sim in similar_artists:
                sim_artist_dict = {}
                sim_artist_dict['artist'] = artist
                sim_artist_dict['cosine_sim'] = cosine_sim
                sim_artist_dict['new locations'] = (
                    pilfer_similar_artist(
                        concerts_df,
                        main_artist,
                        artist,
...
#         (concerts_df['artist'] == artist) &
#           (concerts_df['location'] == location)
#     ]
#     return concerts['date'].max()
# ^ Maybe use this in future iteration
